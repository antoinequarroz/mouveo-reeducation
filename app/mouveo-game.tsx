...
type Arm = "left" | "right";
type ExerciseId = "lateral" | "frontal" | "path" | "hold";
...
const EXERCISES: Record<ExerciseId, { name: string; short: string; instruction: string; color: string }> = {
  lateral: { name: "Bulles latérales", short: "Élévation latérale", instruction: "Écartez le bras sur le côté, puis revenez doucement.", color: "#c4ff4a" },
  frontal: { name: "Lumières devant", short: "Élévation frontale", instruction: "Levez le bras devant vous sans hausser l’épaule.", color: "#7dd3fc" },
  path: { name: "Chemin lumineux", short: "Trajectoire contrôlée", instruction: "Suivez les cibles successives avec un mouvement fluide.", color: "#c4b5fd" },
  hold: { name: "Étoile stable", short: "Maintien contrôlé", instruction: "Atteignez la cible et maintenez la position une seconde.", color: "#fcd34d" },
};
...
export default function MouveoGame() {
  ...
  settingsRef.current = { exercise, arm, amplitude, goal, seated, voice };
  ...
  const recordHit = useCallback(() => {
    ...
    setPhase("return");
    setStatus("Cible atteinte — revenez près de la hanche");
    if ([3, 5, 8].includes(nextCombo)) celebrate(nextCombo === 3 ? "Série lancée !" : nextCombo === 5 ? "Très régulier !" : "Mission accomplie !");
  ...
  const drawScene = useCallback((landmarks?: NormalizedLandmark[][]) => {
    ...
    let shoulder = { x: rect.width / 2 + (cfg.arm === "left" ? -42 : 42), y: rect.height * .45 };
    let hip = { x: shoulder.x, y: rect.height * .72 };
    let wrist: Point | null = null;
    let armLength = Math.min(rect.width, rect.height) * .28;
    let isVisible = demo;

    if (points?.[shoulderIndex] && points?.[wristIndex] && points?.[hipIndex]) {
      shoulder = project(points[shoulderIndex], rect.width, rect.height);
      hip = project(points[hipIndex], rect.width, rect.height);
      wrist = project(points[wristIndex], rect.width, rect.height);
      const elbow = project(points[elbowIndex], rect.width, rect.height);
      armLength = Math.hypot(shoulder.x - elbow.x, shoulder.y - elbow.y) + Math.hypot(elbow.x - wrist.x, elbow.y - wrist.y);
    ...
    const reachTarget = { x: shoulder.x + side * armLength * offset.x * strength, y: shoulder.y + armLength * offset.y * strength };
    const returnTarget = { x: hip.x + side * armLength * .1, y: hip.y };
    ...
    if (!wrist || !isVisible || stageRef.current !== "playing") return;
    ctx.beginPath(); ctx.arc(wrist.x, wrist.y, 11, 0, Math.PI * 2); ctx.fillStyle = "#fff"; ctx.fill();
    const distance = Math.hypot(wrist.x - target.x, wrist.y - target.y);
    if (distance < Math.max(52, armLength * .18)) {
      if (phaseRef.current === "return") completeReturn();
      else if (cfg.exercise === "hold") {
        if (!dwellRef.current) dwellRef.current = Date.now();
        const dwell = Math.min(1, (Date.now() - dwellRef.current) / 1000);
  ...
  const prepareSession = () => {
    stopCamera(); setHits(0); setPoints(0); setCombo(0); comboRef.current = 0; setSeconds(60); setSaved(false); setPain(0); setFatigue(2); setCalibration(0); calibrationRef.current = 0; targetIndexRef.current = 0; hitTimesRef.current = []; phaseRef.current = "reach"; setPhase("reach"); setBodyVisible(true); visibleRef.current = true;
  };
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      setStage("calibrate"); setStatus(seated ? "Asseyez-vous au centre, bras et hanches visibles" : "Reculez : tête, bras et hanches doivent être visibles");
    ...
    <main className="min-h-dvh bg-[#07111f] text-white">
      ...
      <section className="mx-auto grid max-w-7xl gap-4 px-4 pb-6 sm:px-8 lg:grid-cols-[minmax(0,1fr)_340px]">
        <div className="camera-shell relative min-h-[64dvh] overflow-hidden rounded-[2rem] border border-white/10 bg-[#0d1b2d] shadow-2xl">
          ...
          {stage === "finished" && <Summary hits={hits} points={points} regularity={regularity} pain={pain} setPain={setPain} fatigue={fatigue} setFatigue={setFatigue} saved={saved} onSave={saveSummary} onReset={reset} />}
        </div>
...
function Summary({ hits, points, regularity, pain, setPain, fatigue, setFatigue, saved, onSave, onReset }: { hits: number; points: number; regularity: number; pain: number; setPain: (v: number) => void; fatigue: number; setFatigue: (v: number) => void; saved: boolean; onSave: () => void; onReset: () => void }) {
  const stars = hits >= 8 ? 3 : hits >= 5 ? 2 : hits >= 3 ? 1 : 0;
