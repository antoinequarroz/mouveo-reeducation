...
function Welcome({ exercise, setExercise, sessionMode, setSessionMode, programPlan, world, setWorld, painBefore, setPainBefore, cameraError, onCamera, onDemo }: { exercise: ExerciseId; setExercise: (id: ExerciseId) => void; sessionMode: SessionMode; setSessionMode: (mode: SessionMode) => void; programPlan: ExerciseId[]; world: WorldId; setWorld: (world: WorldId) => void; painBefore: number; setPainBefore: (value: number) => void; cameraError: string; onCamera: () => void; onDemo: () => void }) {
  ...
  return <div className="absolute inset-0 flex flex-col"><div className="min-h-0 flex-1 overflow-y-auto px-5 pt-5 sm:px-8 sm:pt-8"><div className="mx-auto max-w-2xl pb-6"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold uppercase tracking-[.18em] text-[#c4ff4a]">Séance du jour</p><h1 className="mt-2 text-4xl font-black leading-tight sm:text-5xl">Bougez à votre rythme.</h1><p className="mt-2 text-base text-slate-300">Choisissez le parcours guidé ou un exercice court.</p></div><span className="hidden rounded-2xl bg-white/5 px-4 py-3 text-3xl sm:block" aria-hidden="true">{WORLDS[world].emoji}</span></div><button onClick={chooseCircuit} aria-pressed={sessionMode === "circuit"} className={`mt-5 w-full rounded-2xl border p-4 text-left transition ${sessionMode === "circuit" ? "border-[#c4ff4a] bg-[#c4ff4a]/12 shadow-[0_0_0_3px_rgba(196,255,74,.06)]" : "border-white/10 bg-gradient-to-r from-sky-400/10 to-violet-400/10 hover:border-white/25"}`}><span className="flex items-center justify-between gap-3"><span className="text-lg font-black">Parcours recommandé</span><span className={`grid size-7 place-items-center rounded-full ${sessionMode === "circuit" ? "bg-[#c4ff4a] text-[#07111f]" : "bg-white/10 text-slate-400"}`}>{sessionMode === "circuit" ? <Check className="size-4" /> : <Sparkles className="size-4" />}</span></span><span className="mt-1 block text-sm leading-relaxed text-slate-300">{programPlan.map((id) => EXERCISES[id].name).join(" → ")}</span><span className="mt-3 inline-block rounded-full bg-white/10 px-3 py-1 text-xs font-bold">Environ {programPlan.length} minute{programPlan.length === 1 ? "" : "s"} · pauses incluses</span></button><div className="mt-6 flex items-center justify-between gap-3"><div><p className="font-black">Exercice au choix</p><p className="text-sm text-slate-400">Une mission d’environ une minute</p></div><span className="text-2xl" aria-hidden="true">{WORLDS[world].emoji}</span></div><div className="mt-3 flex gap-2 overflow-x-auto pb-1" aria-label="Choisir un univers">{(Object.entries(WORLDS) as [WorldId, typeof WORLDS[WorldId]][]).map(([id, item]) => <button key={id} onClick={() => setWorld(id)} aria-pressed={world === id} className={`shrink-0 rounded-full border px-4 py-2 text-sm font-bold ${world === id ? "border-white/40 bg-white/15 text-white" : "border-white/10 text-slate-400 hover:text-white"}`}>{item.emoji} {item.name}</button>)}</div><p className="mt-2 text-sm text-slate-400">{WORLDS[world].description}</p><div className="mt-3 grid grid-cols-2 gap-3">{(Object.entries(EXERCISES) as [ExerciseId, typeof EXERCISES[ExerciseId]][]).filter(([, item]) => item.world === world).map(([id, item]) => { const selected = sessionMode === "single" && exercise === id; return <button key={id} onClick={() => { setExercise(id); setSessionMode("single"); }} aria-pressed={selected} className={`relative min-h-28 rounded-2xl border p-4 text-left transition ${selected ? "border-[#c4ff4a] bg-[#c4ff4a]/10 shadow-[0_0_0_3px_rgba(196,255,74,.05)]" : "border-white/10 bg-white/5 hover:bg-white/10"}`}><span className="mb-3 block size-3 rounded-full" style={{ background: item.color }} />{selected && <Check className="absolute right-3 top-3 size-4 text-[#c4ff4a]" />}<span className="block text-base font-bold leading-tight">{item.name}</span><span className="mt-1 block text-xs leading-relaxed text-slate-400">{item.short}</span></button>; })}</div><label className="mt-5 block rounded-2xl border border-white/10 bg-white/5 p-4"><span className="flex justify-between font-bold"><span>Douleur maintenant</span><span className={painBefore >= 5 ? "text-amber-300" : "text-[#c4ff4a]"}>{painBefore}/10</span></span><input aria-label="Douleur maintenant" className="mt-3 w-full accent-[#c4ff4a]" type="range" min="0" max="10" value={painBefore} onChange={(event) => setPainBefore(Number(event.target.value))} />{painBefore >= 5 && <span className="mt-2 block text-sm leading-relaxed text-amber-200">Réduisez l’amplitude et suivez les consignes de votre professionnel.</span>}</label>{cameraError && <p role="alert" className="mt-4 rounded-xl bg-red-500/15 p-3 text-sm text-red-200">{cameraError}</p>}</div></div><div className="shrink-0 border-t border-white/10 bg-[#07111f]/94 px-5 py-3 backdrop-blur-xl sm:px-8"><div className="mx-auto flex max-w-2xl items-center gap-2"><div className="hidden min-w-0 flex-1 sm:block"><p className="truncate text-sm font-bold">{selectedLabel}</p><p className="text-xs text-slate-400">Prêt quand vous l’êtes</p></div><Button size="lg" onClick={onCamera} className="h-13 flex-1 rounded-2xl bg-[#c4ff4a] px-5 text-base font-black text-[#07111f] hover:bg-[#d5ff7d]"><Camera /> Démarrer la séance</Button><Button id="start-demo" size="lg" variant="outline" onClick={onDemo} aria-label="Essayer sans caméra" title="Essayer sans caméra" className="h-13 rounded-2xl border-white/15 bg-white/5 px-4 text-white hover:bg-white/10 hover:text-white"><Sparkles /><span className="hidden md:inline">Essayer sans caméra</span></Button></div></div></div>;
}
