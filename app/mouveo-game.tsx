...
import { Activity, ArrowRight, BarChart3, Camera, Check, ChevronLeft, Flower2, Footprints, History, Move3D, Orbit, Pause, Play, RotateCcw, Settings2, Shield, Smartphone, Sparkles, Sprout, Star, Trophy, UserRound, Volume2, VolumeX, Waves, Zap } from "lucide-react";
...
export default function MouveoGame() {
  ...
  const recordHit = useCallback(() => {
    ...
    if (nextCombo % 3 === 0 || nextCombo === settingsRef.current.goal) speak(`Série de ${nextCombo}. Revenez doucement.`);
  ...
  return (
    <main className={`app-shell min-h-dvh text-white world-${world}`}>
      ...
      <section className="mx-auto grid max-w-7xl gap-4 px-4 pb-6 sm:px-8 lg:grid-cols-[minmax(0,1fr)_340px]">
        <div className={`camera-shell world-stage world-${world} relative min-h-[64dvh] overflow-hidden rounded-[2rem] bg-[#0d1b2d] shadow-2xl`}>
          ...
          {stage === "playing" && <GameHud points={points} combo={combo} seconds={seconds} status={status} visible={bodyVisible || demo} phase={phase} exercise={exercise} mission={circuitIndex + 1} missionTotal={sessionMode === "circuit" ? programPlan.length : 1} quality={liveQuality} demo={demo} />}
        ...
        <aside className="flex flex-col gap-4">
          ...
          <SessionCard exercise={exercise} arm={arm} setArm={setArm} amplitude={amplitude} setAmplitude={setAmplitude} goal={goal} hits={hits} points={points} combo={combo} adaptiveLevel={adaptiveLevel} sessionMode={sessionMode} circuitIndex={circuitIndex} circuitTotal={programPlan.length} locked={!(["welcome", "setup", "finished"] as Stage[]).includes(stage)} />
...
function Header({ onHistory, onTherapist, voice, voiceProfile, onVoice, onTestVoice, online, canInstall, onInstall }: { onHistory: () => void; onTherapist: () => void; voice: boolean; voiceProfile: string; onVoice: () => void; onTestVoice: () => void; online: boolean; canInstall: boolean; onInstall: () => void }) {
  return <header className="relative z-20 mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-4 sm:px-8 sm:py-5"><div className="flex items-center gap-3"><span className="brand-mark grid size-11 place-items-center rounded-[1.1rem] bg-[#c4ff4a] text-[#07111f]"><Activity className="size-6" strokeWidth={2.5} /></span><div><p className="text-lg font-black tracking-[.04em]">MOUVÉO</p><p className="flex items-center gap-1.5 text-xs text-slate-400"><span className={`size-1.5 rounded-full ${online ? "bg-emerald-300 shadow-[0_0_8px_rgba(110,231,183,.8)]" : "bg-amber-300"}`} />{online ? "Votre séance est prête" : "Disponible hors ligne"}</p></div></div><nav className="nav-cluster flex items-center gap-1 rounded-2xl bg-white/[.045] p-1 shadow-[0_0_0_1px_oklch(1_0_0/.08)]">{canInstall && <Button variant="outline" onClick={onInstall} className="pressable hidden border-[#c4ff4a]/30 bg-[#c4ff4a]/10 text-[#c4ff4a] hover:bg-[#c4ff4a]/20 hover:text-[#c4ff4a] sm:inline-flex"><Sparkles /> Installer</Button>}<Button aria-label={voice ? `Désactiver ${voiceProfile}` : `Activer ${voiceProfile}`} title={voiceProfile} variant="ghost" onClick={onVoice} className="pressable text-slate-300 hover:bg-white/10 hover:text-white">{voice ? <Volume2 /> : <VolumeX />}<span className="hidden text-xs font-bold md:inline">Christophe</span></Button><Button aria-label="Tester la voix Christophe" title="Tester Christophe" variant="ghost" onClick={onTestVoice} disabled={!voice} className="pressable hidden text-[#c4ff4a] hover:bg-[#c4ff4a]/10 hover:text-[#d5ff7d] sm:inline-flex"><Sparkles /><span className="hidden text-xs font-bold lg:inline">Tester</span></Button><Button aria-label="Voir l’historique" variant="ghost" onClick={onHistory} className="pressable text-slate-300 hover:bg-white/10 hover:text-white"><History /> <span className="hidden sm:inline">Progrès</span></Button><Button aria-label="Ouvrir l’espace thérapeute" variant="ghost" onClick={onTherapist} className="pressable text-slate-300 hover:bg-white/10 hover:text-white"><Settings2 /> <span className="hidden sm:inline">Thérapeute</span></Button></nav></header>;
}
...
function Welcome({ exercise, setExercise, sessionMode, setSessionMode, programPlan, world, setWorld, painBefore, setPainBefore, cameraError, onCamera, onDemo }: { exercise: ExerciseId; setExercise: (id: ExerciseId) => void; sessionMode: SessionMode; setSessionMode: (mode: SessionMode) => void; programPlan: ExerciseId[]; world: WorldId; setWorld: (world: WorldId) => void; painBefore: number; setPainBefore: (value: number) => void; cameraError: string; onCamera: () => void; onDemo: () => void }) {
  ...
  const selectedLabel = sessionMode === "circuit" ? `Parcours de ${programPlan.length} exercices` : EXERCISES[exercise].name;
  const WorldIcon = world === "garden" ? Flower2 : world === "space" ? Orbit : Waves;
  return <div className="welcome-screen absolute inset-0 flex flex-col"><div className="min-h-0 flex-1 overflow-y-auto px-5 pt-5 sm:px-8 sm:pt-8"><div className="relative z-10 mx-auto max-w-2xl pb-8"><div className="flex items-start justify-between gap-4"><div><span className="eyebrow-pill"><Sparkles className="size-3.5" /> Séance du jour</span><h1 className="mt-4 max-w-lg text-4xl font-black leading-[1.02] tracking-[-.04em] sm:text-6xl">Votre mouvement devient un jeu.</h1><p className="mt-3 max-w-lg text-base leading-relaxed text-slate-300">Christophe vous guide, à votre rythme, dans un parcours court et adapté.</p></div><span className="hero-orb hidden shrink-0 sm:grid" aria-hidden="true"><WorldIcon className="size-9" strokeWidth={1.7} /></span></div><button onClick={chooseCircuit} aria-pressed={sessionMode === "circuit"} className={`choice-card pressable mt-6 w-full rounded-3xl p-5 text-left ${sessionMode === "circuit" ? "is-selected" : ""}`}><span className="flex items-start justify-between gap-4"><span><span className="block text-xs font-black uppercase tracking-[.14em] text-[#c4ff4a]">Le meilleur choix pour aujourd’hui</span><span className="mt-1 block text-xl font-black">Parcours guidé</span></span><span className={`selection-mark ${sessionMode === "circuit" ? "is-active" : ""}`}>{sessionMode === "circuit" ? <Check className="size-4" /> : <ArrowRight className="size-4" />}</span></span><span className="mt-4 flex flex-wrap items-center gap-2">{programPlan.map((id, index) => <span key={id} className="route-chip"><span>{index + 1}</span>{EXERCISES[id].short}</span>)}</span><span className="mt-4 flex items-center gap-2 text-sm text-slate-300"><Activity className="size-4 text-sky-300" /> {programPlan.length} mini-jeux · pauses incluses · environ {programPlan.length} min</span></button><div className="mt-7 flex items-end justify-between gap-3"><div><p className="text-lg font-black">Ou choisissez votre jeu</p><p className="mt-1 text-sm text-slate-400">Un exercice ciblé d’environ une minute</p></div><span className="text-xs font-bold uppercase tracking-[.12em] text-white/45">{WORLDS[world].description}</span></div><div className="world-switcher mt-4 grid grid-cols-3 gap-1.5 rounded-[1.35rem] bg-black/20 p-1.5 shadow-[inset_0_0_0_1px_oklch(1_0_0/.06)]" aria-label="Choisir un univers">{(Object.entries(WORLDS) as [WorldId, typeof WORLDS[WorldId]][]).map(([id, item]) => { const Icon = id === "garden" ? Flower2 : id === "space" ? Orbit : Waves; return <button key={id} onClick={() => setWorld(id)} aria-pressed={world === id} className={`pressable flex min-h-11 items-center justify-center gap-2 rounded-2xl px-3 text-sm font-bold ${world === id ? "bg-white/14 text-white shadow-[0_0_0_1px_oklch(1_0_0/.12),0_8px_20px_oklch(0_0_0/.18)]" : "text-slate-500 hover:bg-white/[.05] hover:text-white"}`}><Icon className="size-4" />{item.name}</button>; })}</div><div className="mt-3 grid grid-cols-2 gap-3">{(Object.entries(EXERCISES) as [ExerciseId, typeof EXERCISES[ExerciseId]][]).filter(([, item]) => item.world === world).map(([id, item]) => { const selected = sessionMode === "single" && exercise === id; return <button key={id} onClick={() => { setExercise(id); setSessionMode("single"); }} aria-pressed={selected} className={`exercise-tile pressable relative min-h-32 rounded-[1.35rem] p-4 text-left ${selected ? "is-selected" : ""}`}><span className="exercise-glyph" style={{ color: item.color, background: `${item.color}18`, boxShadow: `0 0 0 1px ${item.color}28` }}>{item.family === "jambes" ? <Footprints className="size-5" /> : item.family === "équilibre" ? <Shield className="size-5" /> : <Activity className="size-5" />}</span><span className={`selection-mark absolute right-3 top-3 ${selected ? "is-active" : ""}`}>{selected ? <Check className="size-4" /> : <ArrowRight className="size-4" />}</span><span className="mt-4 block pr-7 text-base font-black leading-tight">{item.name}</span><span className="mt-1 block text-xs leading-relaxed text-slate-400">{item.short}</span></button>; })}</div><label className={`pain-card mt-5 block rounded-[1.35rem] p-4 ${painBefore >= 5 ? "is-alert" : ""}`}><span className="flex items-center justify-between font-bold"><span><span className="block">Comment vous sentez-vous ?</span><span className="mt-0.5 block text-xs font-normal text-slate-400">Douleur ressentie avant la séance</span></span><span className={`pain-score ${painBefore >= 5 ? "text-amber-200" : "text-[#c4ff4a]"}`}>{painBefore}<small>/10</small></span></span><input aria-label="Douleur maintenant" className="mt-3 w-full accent-[#c4ff4a]" type="range" min="0" max="10" value={painBefore} onChange={(event) => setPainBefore(Number(event.target.value))} />{painBefore >= 5 && <span className="mt-2 block text-sm leading-relaxed text-amber-200">Réduisez l’amplitude et suivez les consignes de votre professionnel.</span>}</label>{cameraError && <p role="alert" className="mt-4 rounded-xl bg-red-500/15 p-3 text-sm text-red-200">{cameraError}</p>}</div></div><div className="action-dock shrink-0 px-5 py-3 sm:px-8"><div className="mx-auto flex max-w-2xl items-center gap-2"><div className="hidden min-w-0 flex-1 sm:block"><p className="truncate text-sm font-black">{selectedLabel}</p><p className="text-xs text-slate-400">Prêt quand vous l’êtes</p></div><Button size="lg" onClick={onCamera} className="primary-action pressable h-14 flex-1 rounded-2xl bg-[#c4ff4a] px-5 text-base font-black text-[#07111f] hover:bg-[#d5ff7d]"><Camera /> Commencer <ArrowRight className="size-4" /></Button><Button id="start-demo" size="lg" variant="outline" onClick={onDemo} aria-label="Essayer sans caméra" title="Essayer sans caméra" className="pressable h-14 rounded-2xl border-white/15 bg-white/5 px-4 text-white hover:bg-white/10 hover:text-white"><Play className="size-4" /><span className="hidden md:inline">Mode démo</span></Button></div></div></div>;
}
...
function RestScreen({ completed, total, nextExercise, onContinue, onStop }: { completed: number; total: number; nextExercise: ExerciseId; onContinue: () => void; onStop: () => void }) {
  return <div className="absolute inset-0 z-30 grid place-items-center bg-[#07111f]/92 p-6 backdrop-blur"><div className="max-w-md text-center"><span className="mx-auto grid size-16 place-items-center rounded-full bg-sky-300/15 text-3xl">💧</span><p className="mt-5 text-sm font-bold uppercase tracking-[.18em] text-sky-300">Pause récupération · {completed}/{total}</p><h2 className="mt-2 text-4xl font-black">Respirez tranquillement.</h2><p className="mt-3 text-slate-300">Prochain exercice : <strong className="text-white">{EXERCISES[nextExercise].name}</strong></p><div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center"><Button size="lg" onClick={onContinue} className="h-12 rounded-xl bg-[#c4ff4a] font-bold text-[#07111f] hover:bg-[#d5ff7d]"><Play /> Continuer le parcours</Button><Button size="lg" variant="outline" onClick={onStop} className="h-12 rounded-xl border-white/15 bg-white/5 text-white hover:bg-white/10 hover:text-white">Terminer ici</Button></div></div></div>;
}

function GameHud({ points, combo, seconds, status, visible, phase, exercise, mission, missionTotal, quality, demo }: { points: number; combo: number; seconds: number; status: string; visible: boolean; phase: Phase; exercise: ExerciseId; mission: number; missionTotal: number; quality: LiveQuality; demo: boolean }) {
  const clock = `${Math.floor(seconds / 60)}:${String(seconds % 60).padStart(2, "0")}`;
  const shown = demo ? { amplitude: 100, stability: "stable" as const, tempo: "régulier" as const } : quality;
  return <><div className="pointer-events-none absolute inset-x-0 top-0 flex items-start justify-between bg-gradient-to-b from-black/80 to-transparent p-4 pb-24 sm:p-5"><div className="min-w-0 pr-3"><p className="mb-1 truncate text-xs font-bold uppercase tracking-[.12em] text-[#c4ff4a]">{missionTotal > 1 ? `Étape ${mission}/${missionTotal} · ` : ""}{EXERCISES[exercise].name}</p><div className="flex items-center gap-3"><div><p className="text-xs text-white/70">Score</p><p className="text-3xl font-black tabular-nums sm:text-4xl">{points}</p></div>{combo >= 3 && <div className="h-fit rounded-full border border-amber-300/30 bg-amber-300/15 px-3 py-1 text-sm font-black text-amber-200"><Zap className="mr-1 inline size-4 fill-current" /> ×{combo}</div>}</div></div><div className="rounded-2xl bg-black/45 px-4 py-2 text-center backdrop-blur sm:px-5 sm:py-3"><p className="text-xs text-white/70">Temps</p><p className="text-xl font-black tabular-nums sm:text-2xl">{clock}</p></div></div><div className="pointer-events-none absolute inset-x-0 bottom-4 flex justify-center px-3"><div className={`w-full max-w-xl rounded-2xl px-4 py-3 text-center shadow-xl backdrop-blur ${visible ? "bg-[#07111f]/88 text-white" : "bg-amber-300 text-[#07111f]"}`}><p className="text-sm font-bold">{visible ? <>{phase === "reach" ? <Play className="mr-2 inline size-4" /> : <RotateCcw className="mr-2 inline size-4" />}{status}</> : <><Pause className="mr-2 inline size-4" />Repositionnez-vous — séance en pause</>}</p>{visible && <div className="mt-2 grid grid-cols-3 gap-1 border-t border-white/10 pt-2 text-xs"><span><strong className="text-[#c4ff4a]">{shown.stability === "waiting" ? "—" : `${Math.min(100, shown.amplitude)}%`}</strong><span className="ml-1 text-white/55">amplitude</span></span><span><strong className={shown.stability === "adjust" ? "text-amber-200" : "text-emerald-200"}>{shown.stability === "adjust" ? "À ajuster" : shown.stability === "stable" ? "Stable" : "—"}</strong></span><span><strong className={shown.tempo === "rapide" ? "text-amber-200" : "text-sky-200"}>{shown.tempo === "attente" ? "—" : shown.tempo}</strong><span className="ml-1 text-white/55">rythme</span></span></div>}</div></div></>;
}
...
function SessionCard({ exercise, arm, setArm, amplitude, setAmplitude, goal, hits, points, combo, adaptiveLevel, sessionMode, circuitIndex, circuitTotal, locked }: { exercise: ExerciseId; arm: Arm; setArm: (v: Arm) => void; amplitude: number; setAmplitude: (v: number) => void; goal: number; hits: number; points: number; combo: number; adaptiveLevel: number; sessionMode: SessionMode; circuitIndex: number; circuitTotal: number; locked: boolean }) {
  return <div className="rounded-[1.6rem] border border-white/10 bg-white/[.055] p-5"><div className="flex items-center justify-between gap-2"><p className="font-bold">{sessionMode === "circuit" ? `Parcours · exercice ${circuitIndex + 1}/${circuitTotal}` : "Exercice sélectionné"}</p><span className="rounded-full bg-[#c4ff4a]/15 px-3 py-1 text-xs font-bold uppercase text-[#c4ff4a]">{EXERCISES[exercise].family}</span></div><h2 className="mt-4 text-2xl font-black">{EXERCISES[exercise].name}</h2><p className="mt-1 text-sm leading-relaxed text-slate-400">{EXERCISES[exercise].instruction}</p>{EXERCISES[exercise].mode !== "both" && <div className="mt-4 flex gap-2">{(["left","right"] as Arm[]).map((value) => <button key={value} disabled={locked} onClick={() => setArm(value)} className={`flex-1 rounded-xl border px-3 py-2 text-sm font-bold disabled:cursor-not-allowed disabled:opacity-55 ${arm === value ? "border-sky-300 bg-sky-300/15 text-sky-200" : "border-white/10 text-slate-400"}`}>Côté {value === "left" ? "gauche" : "droit"}</button>)}</div>}<label className="mt-4 flex justify-between text-sm"><span className="text-slate-400">Amplitude confortable</span><span className="font-bold">{amplitude}%</span></label><input aria-label="Amplitude confortable" disabled={locked} className="mt-2 w-full accent-[#c4ff4a] disabled:cursor-not-allowed disabled:opacity-55" type="range" min="55" max="95" step="10" value={amplitude} onChange={(e) => setAmplitude(Number(e.target.value))} />{locked && <p className="mt-2 text-xs text-slate-500">Ces réglages restent fixes pendant la séance.</p>}<div className="mt-4"><div className="mb-2 flex justify-between text-sm"><span className="text-slate-400">Progression</span><span className="font-bold">{Math.min(goal, hits)}/{goal}</span></div><Progress value={Math.min(100, hits / goal * 100)} className="h-2 bg-white/10 [&_[data-slot=progress-indicator]]:bg-[#c4ff4a]" /></div><div className="mt-4 grid grid-cols-3 gap-2 text-center"><Metric value={String(points)} label="points" /><Metric value={`×${combo}`} label="série" /><Metric value={String(adaptiveLevel)} label="niveau adapté" /></div><p className="mt-3 text-xs leading-relaxed text-slate-500">La cible s’ajuste légèrement selon votre régularité, sans dépasser l’amplitude choisie.</p></div>;
}
