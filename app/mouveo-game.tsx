...
const CIRCUIT: ExerciseId[] = ["lateral", "march", "balance"];
...
export default function MouveoGame() {
  ...
  const sessionGoal = goal * (sessionMode === "circuit" ? CIRCUIT.length : 1);
  ...
  const recordHit = useCallback(() => {
    ...
    setHits((value) => {
      ...
      if (next >= settingsRef.current.goal) {
        window.setTimeout(() => {
          if (settingsRef.current.sessionMode === "circuit" && circuitIndexRef.current < CIRCUIT.length - 1) {
            setStage("rest");
  ...
  const continueCircuit = () => {
    ...
    circuitIndexRef.current = nextIndex; setCircuitIndex(nextIndex); setExercise(CIRCUIT[nextIndex]); setHits(0); targetIndexRef.current = 0; dwellRef.current = 0; returnStartedRef.current = 0; phaseRef.current = "reach"; setPhase("reach"); setStatus("Nouvelle mission — commencez lentement"); setStage("playing"); speak(`Jeu ${nextIndex + 1} sur ${CIRCUIT.length}. ${EXERCISES[CIRCUIT[nextIndex]].instruction}`);
  };
  ...
  return (
    <main className="min-h-dvh bg-[#07111f] text-white">
      <Header onHistory={() => setView("history")} onTherapist={() => { setSessionMode("single"); setView("therapist"); }} voice={voice} onVoice={() => setVoice((v) => !v)} />
      <section className="mx-auto grid max-w-7xl gap-4 px-4 pb-6 sm:px-8 lg:grid-cols-[minmax(0,1fr)_340px]">
        <div className="camera-shell relative min-h-[64dvh] overflow-hidden rounded-[2rem] border border-white/10 bg-[#0d1b2d] shadow-2xl">
          ...
          {stage === "welcome" && <Welcome exercise={exercise} setExercise={setExercise} sessionMode={sessionMode} setSessionMode={setSessionMode} world={world} setWorld={setWorld} painBefore={painBefore} setPainBefore={setPainBefore} cameraError={cameraError} onCamera={startCamera} onDemo={startDemo} />}
          ...
          {stage === "rest" && <RestScreen completed={circuitIndex + 1} nextExercise={CIRCUIT[circuitIndex + 1]} onContinue={continueCircuit} onStop={() => { stopCamera(); setStage("finished"); }} />}
        ...
        <aside className="flex flex-col gap-4">
          <SessionCard exercise={exercise} arm={arm} setArm={setArm} amplitude={amplitude} setAmplitude={setAmplitude} goal={goal} hits={hits} points={points} combo={combo} adaptiveLevel={adaptiveLevel} sessionMode={sessionMode} circuitIndex={circuitIndex} />
...
function RestScreen({ completed, nextExercise, onContinue, onStop }: { completed: number; nextExercise: ExerciseId; onContinue: () => void; onStop: () => void }) {
  return <div className="absolute inset-0 z-30 grid place-items-center bg-[#07111f]/92 p-6 backdrop-blur"><div className="max-w-md text-center"><span className="mx-auto grid size-16 place-items-center rounded-full bg-sky-300/15 text-3xl">💧</span><p className="mt-5 text-sm font-bold uppercase tracking-[.18em] text-sky-300">Pause récupération · {completed}/3</p><h2 className="mt-2 text-4xl font-black">Respirez tranquillement.</h2><p className="mt-3 text-slate-300">Prochaine mission : <strong className="text-white">{EXERCISES[nextExercise].name}</strong></p><div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center"><Button size="lg" onClick={onContinue} className="h-12 rounded-xl bg-[#c4ff4a] font-bold text-[#07111f] hover:bg-[#d5ff7d]"><Play /> Je suis prêt</Button><Button size="lg" variant="outline" onClick={onStop} className="h-12 rounded-xl border-white/15 bg-white/5 text-white hover:bg-white/10 hover:text-white">Terminer ici</Button></div></div></div>;
}
